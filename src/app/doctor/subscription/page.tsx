  ...
  const handleSubscribe = async (planName: string, amount: number) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Subscription created! Payment reference: ${data.paymentReference}`);
        fetchSubscriptionData();
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage your DFC Medical subscription and billing</p>
        </div>
      </div>
            ...
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <h3 className="text-xl font-semibold">{currentSubscription.planName} Plan</h3>
          ...
          {SUBSCRIPTION_PLANS.map((plan) => (
            <Card key={plan.name} className={`relative ${plan.popular ? 'border-blue-500 shadow-lg' : ''}`}>
              ...
              <CardContent>
                ...
                >
                  {currentSubscription?.status === 'ACTIVE' ? 'Current Plan' : 'Subscribe'}
                </Button>
              </CardContent>
            </Card>
          ))}
      ...
      {!currentSubscription && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <Button onClick={() => document.querySelector('.grid')?.scrollIntoView({ behavior: 'smooth' })}>
              View Plans
            </Button>
          </CardContent>
        </Card>
